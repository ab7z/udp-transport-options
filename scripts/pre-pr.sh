...
FUZZ_TARGETS=(wire_datagram options_tlv options_serialize options_ocs options_typed socket_assemble process_datagram frag_split frag_reassembly)
...
for target in "${FUZZ_TARGETS[@]}"; do
    ...
    if [ "$target" = "options_serialize" ]; then
        ...
        max_len=384
    elif [ "$target" = "frag_reassembly" ]; then
        max_len=512
    fi
