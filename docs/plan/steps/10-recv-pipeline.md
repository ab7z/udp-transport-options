...
- `process_datagram(ip_datagram: &[u8], cache: &mut ReassemblyCache, now: Instant) ->
  Result<Delivery, RecvError>` (the current signature of `architecture.md`: the pipeline takes the
  full IP-datagram bytes plus caller-driven reassembly time).
...
- Order: verify the UDP checksum, locate/validate the surplus area, validate the OCS, parse the
  options, then either pass a valid empty-payload FRAG to the explicit reassembly cache or deliver.
...
1. `process_datagram(ip_datagram, &mut cache, now)`: check the UDP Length bounds (drop + log outside
...
4. On a clean FRAG with empty user data, insert into the explicit `ReassemblyCache`; incomplete
   reassembly returns `Delivery::Buffered`, cache-side aborts return `Delivery::Dropped`, and a
   complete reassembly is re-processed exactly once. On FRAG with non-empty user data, ignore all
   options and deliver the user data.
...
- `process_datagram` is fuzzed for no-panic behavior and a strengthened disposition oracle; Step 12
  extends the same signature with live reassembly state and a single re-process path.
