...
is the job of `wire::surplus`, whose `SurplusLayout { starts_at, needs_pad, len }` records the even
start offset, whether a pad byte is required, and the total surplus length.
...
The OCS MUST be the first option in the surplus area, and it MUST start on an even offset relative to
the start of the UDP header (RFC 9868 Sec. 8). The surplus area is interpreted as UDP options only
when it is at least 2 bytes long, the OCS comes first on an even offset, and the OCS validates;
otherwise the entire surplus area is ignored as though no options were present (RFC 9868 Sec. 8).

When the surplus area would otherwise begin at an odd offset relative to the start of the UDP header,
a single pad byte is inserted before the OCS so that the OCS itself starts on an even boundary. This
