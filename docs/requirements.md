...
| FR-37 | Make per-packet options and their success/fail status available to the user, except FRAG, NOP, EOL (handled internally). | Sec. 14, 15 | MUST | in | 10,12,13 | Partially implemented | `Delivery::Payload.options: Vec<RawOption>` surfaces valid APC/REQ/RES/MDS/MRDS; FRAG/NOP/EOL are never surfaced. Step 12 also reports coalesced per-fragment MDS/MRDS minima and the most recently received per-fragment REQ/RES tokens after successful reassembly. Rich success/fail status remains Step 13. |
...
coalesced per-fragment MDS/MRDS/REQ/RES values after successful reassembly, but it does not yet expose rich
success/fail status or per-fragment detail. This is a deliberate Step-13 API reduction in fidelity worth naming
for FF1.
