...
| FR-40 | Build and transmit option-bearing datagrams via a raw `IP_HDRINCL` socket so that UDP Length is strictly less than IP Total Length on the wire (the surplus area exists). | Sec. 7; locked decision | MUST | in | 8 | Partial-in-userspace | `socket::send`. Total Length set explicitly; IP checksum and possibly Identification are filled by the kernel (see limitations). Empty option sets emit a plain UDP datagram with no surplus area. Linux + CAP_NET_RAW only. |
...
| FR-43 | Provide a high-level API that applies the OCS and FRAG fragmentation/reassembly transparently (a send too large for a single datagram auto-fragments, capped by the peer's MRDS; recv reassembles). | Sec. 15 | SHOULD | in | 13 | Implemented | `api::build_outgoing_datagrams()` and `api::Peer` compose serializer, raw sockets, FRAG split/reassembly, and receive policy. Fragmentation is triggered by configured single-datagram capacity, never MRDS; over-MRDS sends fail with `SendError::Split`. Empty `SendOptions` do not add an empty OCS/EOL body, preserving the default IPv4 MRDS payload capacity. Raw FRAG aliases are rejected, raw APC aliases cannot be combined with automatic APC, and duplicate high-level APC/MDS/MRDS/REQ/RES Kinds are rejected. |
| FR-44 | Offer a per-socket-pair receive setting to require named options (drop + log if absent) and a setting to discard all option-bearing datagrams (defaulting to normal processing). | Sec. 15 | MUST | in | 13 | Implemented | `ReceivePolicy` supports successful APC/MDS/MRDS/REQ/RES options from the datagram or coalesced FragmentSet and `drop_all_option_bearing`; a FragmentSet success exists only when no fragment failed that option kind. Drop-all filtering validates the UDP checksum before classifying a usable option-bearing surplus layout as policy-filtered, and policy drops are logged with sampling. Tails too short for the aligned OCS are delivered without options. |
...
| Sec. 15 | Receive API: per-packet/per-fragment required-or-omitted options | MUST | partial | 13 | Per-packet covered and coalesced per-fragment successes can satisfy `ReceivePolicy` (FR-44); per-fragment status reporting remains shallow/default-off in depth. |
| Sec. 15 | Required option absent -> silently drop + log | MUST | yes | 13 | FR-44; implemented for successful datagram-level or coalesced FragmentSet options. |
