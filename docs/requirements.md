...
| FR-37 | Make per-packet options and their success/fail status available to the user, except FRAG, NOP, EOL (handled internally). | Sec. 14, 15 | MUST | in | 10,12,13 | Implemented | `Delivery::Payload.options: Vec<RawOption>` surfaces successful APC/REQ/RES/MDS/MRDS and coalesced per-fragment SAFE options; `OptionReport { kind, status, source }` reports success/failure/ignored status for API consumers. FRAG/NOP/EOL remain internal. |
