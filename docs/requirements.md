...
| FR-25 | Implement REQ (Kind 6, Len 6) and RES (Kind 7, Len 6): a 4-byte opaque token; encode/decode and deliver to the user; never auto-respond. | Sec. 11.7 | MUST | in | 7 | Planned | `typed::Req`, `typed::Res`. Any auto REQ/RES layer (e.g. for DPLPMTUD) is out of scope and would be off by default. A transmitted RES token MUST be a token received in a REQ (Sec. 11.7); enforced at the API tier. |
