...
IDs (FR-NN, NFR-NN) are stable; do not renumber. Steps are the ROADMAP.md numbers 0..18 (16 removed with IPv6; 9 merged into 8). Arrows are written `->`;
comparisons `<=` / `>=`.
...
| FR-34 | Limit reassembly resources per socket pair rather than sharing the resource budget across pairs; abort on exceedance. | Sec. 11.4, 25.4 | SHOULD | in | 12,13 | Partially implemented | Each `Peer` owns one `ReassemblyCache`, and low-level callers MUST not share a cache across pairs because `max_pending_partials` applies to the whole cache. `Peer::bind` however passes only `src_port`/`dst_port` to `RawReceiver` (`own_src = None`), so datagrams of different address pairs that share those ports share the pending-partial budget. Per-datagram size/segment caps and the per-cache pending cap return `LimitExceeded` without evicting unrelated keys. |
...
| NFR-06 | DoS resistance, reassembly: per-pair (non-shared) and per-cache caps, a default timeout `<= 2 min`, garbage collection of stale partials, and overlap-abort. | Sec. 11.4, 25.4 | SHOULD | 12 | Partially implemented | Implements FR-32 and FR-33 fully. FR-34 is partial: `Peer` owns one cache instance, but the raw receiver filters ports only, so distinct address pairs that share those ports share the pending-partial budget (see FR-34). GC is caller-driven and expiry is enforced on insertion; there is no background thread. |
...
| NFR-10 | Performance/overhead observability: make the surplus-area overhead measurable on the wire and provide a reproducible evaluation runbook for FF2. | FF2; ROADMAP Step 17 | SHOULD | 10.5,17 | Partially implemented | The independent wire lane validates endpoint wire images. Step 17 provides controlled local namespace/veth, routed, Linux NAT, and negative filter paths. External campaigns from 2026-08-10 through 2026-08-16 measured public paths; sealed archives and the FF2 answer in the stated scope live in the companion thesis repo (`../mcs-thesis-docs`). This crate still has no committed tunnel lane or surplus-only rewriting middlebox. |
...
| Sec. 11.4 | Reassembly space limited; not shared across pairs | SHOULD | partial | 12,13 | FR-34, NFR-06: one cache per `Peer`, but the raw receiver filters ports only. |
...
is squarely the FF2 measurement target. Step 17 stages controlled namespace/veth, routed, Linux NAT,
and negative filter paths. External campaigns from 2026-08-10 through 2026-08-16 measured public paths
(EU cloud triangle, Telefónica hotspot, GCP, AWS US and AP); sealed archives live in
`../mcs-thesis-docs/thesis/evidence/`. Tunnel behavior remains an encapsulation control, not native
transit. No endpoint code can make a stripping path conformant.
...
modules. RES provenance is deliberately delegated to the upper layer. The broader Sec. 15 API remains partial where
this document says so, and FR-34 is partial because `Peer` filters the raw receiver by ports only.
The raw-socket boundary degrades, but does not block, the I/O requirements (FR-40, FR-41: Partial-in-
userspace, per L1, L4, L5). The only outright infeasibility in this crate is platform reach (macOS receive, L7).
Network-path survival of the surplus area (L3) is the empirical question FF2 measures; the companion thesis
answers it for the observed pairs, directions, and windows rather than treating it as a code defect.
