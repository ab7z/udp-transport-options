...
| FR-22 | Implement APC (Kind 2, Len 6): a CRC32C over the UDP user data only; encode/decode and verify it. | Sec. 11.3 | MUST | in | 7 | Partially implemented | `options::typed::Apc { crc32c }` encodes/decodes Len 6 and computes CRC32C with the `crc32c` crate, validated against `123456789 -> 0xe3069283`; receive-side APC failure indication remains FR-23/Step 10/13. |
| FR-23 | On an incorrect or unrecognized-length APC, default to delivering the payload with an APC-failure indication, ignoring the option (do not drop), unless explicitly configured otherwise. | Sec. 11.3, 14, 19 | MUST/SHOULD | in | 7,10,13 | Partially implemented | Step 10 verifies APC and ignores incorrect/unrecognized APC without dropping data. Rich failure status remains Step 13. |
| FR-24 | Implement MDS (Kind 4, Len 4) and MRDS (Kind 5, Len 5): 16-bit size, and for MRDS an 8-bit segment count; encode/decode and report to the user. | Sec. 11.5, 11.6 | MUST | in | 7 | Partially implemented | `typed::Mds` and `typed::Mrds` encode/decode the fixed wire values. API-level reporting remains Step 13. MDS MUST NOT be used to limit transmission (it is a hint). |
| FR-25 | Implement REQ (Kind 6, Len 6) and RES (Kind 7, Len 6): a 4-byte opaque token; encode/decode and deliver to the user; never auto-respond. | Sec. 11.7 | MUST | in | 7 | Partially implemented | `typed::Req` and `typed::Res` encode/decode the fixed token. API-level delivery and the RES-token policy remain Step 13. Any auto REQ/RES layer (e.g. for DPLPMTUD) is out of scope and would be off by default. |
| FR-26 | Implement the FRAG option (Kind 3): non-terminal Len 10 (Frag Start, Identification, Frag Offset) and terminal Len 12 (adds RDOS). | Sec. 11.4 | MUST | in | 7,10,11,12 | Partially implemented | `typed::Frag { frag_start, identification, frag_offset, rdos }` encodes/decodes both value layouts; `rdos: Some(..)` marks terminal. Step 10 uses `Frag. Start` to stop option parsing before fragment data. Step 11 emits non-terminal and terminal FRAG bodies for send-side fragmentation; receive-side reassembly remains Step 12. |
| FR-27 | Treat a malformed FRAG option as an unsupported UNSAFE option (not as an ignorable SAFE option), after applying the generic Sec. 10 below-minimum Length rule. | Sec. 10, 11.4 | MUST | in | 10,12 | Implemented | Step 10 maps a malformed FRAG at or above the FRAG minimum length, including a `Frag. Start` that points before the end of the valid FRAG option or beyond the datagram, to zero-length delivery even when UDP user data is non-empty. A FRAG Length below 10 is a malformed surplus area, so options are discarded and the original UDP user data is delivered. Step 12 will apply the same rule after actual reassembly exists. |
...
| FR-35 | Support a local MRDS size of at least 2926 (IPv4) bytes and at least 2 segments; assume these as defaults when no MRDS was received. | Sec. 11.4, 11.6 | MUST | in | 11,12 | Partially implemented | `model::limits::{MRDS_DEFAULT_IPV4, MIN_REASSEMBLY_SEGMENTS=2}` and `PeerFragmentLimits::default_ipv4()` cover the send-side default. Receive-side reassembly enforcement remains Step 12. |
...
| FR-37 | Make per-packet options and their success/fail status available to the user, except FRAG, NOP, EOL (handled internally). | Sec. 14, 15 | MUST | in | 10,13 | Partially implemented | `Delivery::Payload.options: Vec<RawOption>` surfaces valid APC/REQ/RES/MDS/MRDS; FRAG/NOP/EOL are never surfaced. Rich success/fail status remains Step 13. |
| FR-38 | Deliver the UDP user data by default for all SAFE options regardless of whether options are supported, present, or succeed (legacy equivalence), unless explicitly overridden. | Sec. 6, 14, 19 | MUST | in | 10,13 | Implemented | Step 10's default path delivers data for SAFE option success/failure; API-level overrides remain Step 13. |
| FR-39 | Silently drop the reassembled user data if any fragment or the datagram carries an unsupported UNSAFE Kind, or an UNSAFE option appears outside a fragment context. | Sec. 10, 12, 14 | MUST | in | 10,12 | Partially implemented | Step 10 drops unsupported UNSAFE outside a valid empty-FRAG context by delivering a zero-length datagram. A valid empty-FRAG datagram with a fragment-local unsupported UNSAFE or malformed per-fragment option returns `Delivery::Dropped`, because the Step 10 cache stub cannot preserve that failure state. Step 12 extends this to persisted fragment-failure state and completed reassembled data. |
...
| FR-43 | Provide a high-level API that applies the OCS and FRAG fragmentation/reassembly transparently (a send too large for a single datagram auto-fragments, capped by the peer's MRDS; recv reassembles). | Sec. 15 | SHOULD | in | 13 | Planned | `api` high tier; default-deliver with optional drop-on-failure override (FR-38). The fragmentation trigger is the single-datagram capacity (path MTU, MDS hint), never MRDS; a send whose reassembled size exceeds the peer's MRDS fails with an error. |
...
| FR-45 | Do not expose user control over option order or per-packet fragment boundaries; do allow enabling/disabling options (incl. fragmentation) per packet. | Sec. 15 (API guidance, non-normative); Sec. 25 (rationale) | guidance | in | 13 | Planned | Sec. 15 states APIs "are not intended to provide" this control (no BCP-14 MUST); enabling/disabling per packet is expected. Enforced here as the project's anti-covert-channel canonicalization. The API toggles options, not layout (FR-14). |
...
| Sec. 10 | Unsupported UNSAFE -> terminate processing, drop all options | MUST | yes | 10,12 | FR-39; valid empty-FRAG datagrams with fragment-local UNSAFE failures produce no user delivery in Step 10 and will gain persisted failure state in Step 12. |
