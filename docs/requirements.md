...
| FR-44 | Offer a per-socket-pair receive setting to require named options (drop + log if absent) and a setting to discard all option-bearing datagrams (defaulting to normal processing). | Sec. 15 | MUST | in | 13 | Implemented | `ReceivePolicy` supports successful required APC/MDS/MRDS/REQ/RES options and `drop_all_option_bearing`; policy drops are logged with sampling and default to normal processing. |
